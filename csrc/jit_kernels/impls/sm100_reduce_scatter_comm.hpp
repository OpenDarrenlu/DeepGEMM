...
class SM100ReduceScatterCommRuntime final: public LaunchRuntime<SM100ReduceScatterCommRuntime> {
public:
    struct Args {
        int shape_m, shape_n;
    ...
    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#include <deep_gemm/impls/sm100_reduce_scatter_comm.cuh>

using namespace deep_gemm;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&sm100_reduce_scatter_comm_impl<
        {}, {}, {}, {}
    >);
}};
)",
...
static void sm100_reduce_scatter_comm(const torch::Tensor& local_scratch,
    ...
    const SM100ReduceScatterCommRuntime::Args args = {
        .shape_m = m, .shape_n = n,
    ...
    const auto code = SM100ReduceScatterCommRuntime::generate(args);
    const auto runtime = compiler->build("sm100_reduce_scatter_comm", code);
    SM100ReduceScatterCommRuntime::launch(runtime, args);
}
